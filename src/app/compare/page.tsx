'use client';

import { useEffect, useState } from 'react';
import Link from 'next/link';

interface Country {
  code: string;
  name: string;
  flag: string;
}

interface Entry {
  topic: string;
  status: 'green' | 'yellow' | 'red';
  plain_explanation: string;
  cultural_note: string | null;
}

interface TopicComparison {
  topic: string;
  entries: { country: Country; entry: Entry | null }[];
}

const statusConfig = {
  green: { icon: '✓', label: 'Permitido', color: '#22c55e', bg: 'bg-green-100' },
  yellow: { icon: '!', label: 'Restrições', color: '#f59e0b', bg: 'bg-yellow-100' },
  red: { icon: '✕', label: 'Proibido', color: '#ef4444', bg: 'bg-red-100' },
};

export default function ComparePage() {
  const [countries, setCountries] = useState<Country[]>([]);
  const [selected, setSelected] = useState<string[]>([]);
  const [comparisons, setComparisons] = useState<TopicComparison[]>([]);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    fetch('/api/countries')
      .then((res) => res.json())
      .then(setCountries);
  }, []);

  // Auto-compare when 2+ countries selected
  useEffect(() => {
    if (selected.length >= 2) {
      compare();
    } else {
      setComparisons([]);
    }
  }, [selected]);

  const toggleCountry = (code: string) => {
    if (selected.includes(code)) {
      setSelected(selected.filter((c) => c !== code));
    } else if (selected.length < 3) {
      setSelected([...selected, code]);
    }
  };

  const compare = async () => {
    setLoading(true);
    const res = await fetch('/api/compare', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ countries: selected }),
    });
    const data = await res.json();
    setComparisons(data.comparisons || []);
    setLoading(false);
  };

  const selectedCountries = countries.filter(c => selected.includes(c.code));

  return (
    <main className="min-h-screen p-6 md:p-8 max-w-6xl mx-auto">
      {/* Disclaimer */}
      <div className="disclaimer">
        <p className="disclaimer-text">
          ⚠️ Informação educacional. Não é aconselhamento jurídico.
        </p>
      </div>

      <Link href="/" className="inline-flex items-center gap-2 text-indigo-500 hover:text-indigo-600 mb-6 font-medium">
        ← Voltar para países
      </Link>

      <h1 className="text-4xl font-bold mb-8 text-center bg-gradient-to-r from-indigo-500 to-purple-600 bg-clip-text text-transparent">
        🔄 Comparar Países
      </h1>

      {/* Country selector */}
      <div className="card p-6 mb-8">
        <label className="block text-sm font-semibold mb-3">
          🌍 Selecione 2 ou 3 países para comparar: 
          <span className="ml-2 px-2 py-1 bg-indigo-100 text-indigo-700 rounded-full text-xs">
            {selected.length}/3
          </span>
        </label>
        <div className="flex flex-wrap gap-2">
          {countries.map((c) => (
            <button
              key={c.code}
              onClick={() => toggleCountry(c.code)}
              className={`category-pill ${
                selected.includes(c.code) ? 'category-pill-active' : 'category-pill-inactive'
              }`}
            >
              {c.flag} {c.name}
            </button>
          ))}
        </div>
      </div>

      {/* Loading */}
      {loading && (
        <div className="text-center py-12">
          <span className="text-4xl animate-spin inline-block">⏳</span>
          <p className="mt-4 text-gray-500">Comparando leis...</p>
        </div>
      )}

      {/* Results Table */}
      {!loading && comparisons.length > 0 && (
        <div className="card overflow-hidden">
          {/* Header with country flags */}
          <div className="grid border-b bg-gray-50" style={{ gridTemplateColumns: `200px repeat(${selectedCountries.length}, 1fr)` }}>
            <div className="p-4 font-bold border-r">Tema</div>
            {selectedCountries.map(c => (
              <div key={c.code} className="p-4 text-center border-r last:border-r-0">
                <span className="text-3xl block mb-1">{c.flag}</span>
                <span className="font-semibold">{c.name}</span>
              </div>
            ))}
          </div>

          {/* Rows */}
          {comparisons.map((comp, i) => (
            <div 
              key={comp.topic} 
              className="grid border-b last:border-b-0 hover:bg-gray-50 transition"
              style={{ gridTemplateColumns: `200px repeat(${selectedCountries.length}, 1fr)` }}
            >
              <div className="p-4 font-medium border-r flex items-center">
                {comp.topic}
              </div>
              {comp.entries.map(({ country, entry }) => {
                const config = entry ? statusConfig[entry.status] : null;
                return (
                  <div 
                    key={country?.code} 
                    className={`p-4 border-r last:border-r-0 ${config?.bg || ''}`}
                  >
                    {entry ? (
                      <div className="flex flex-col items-center text-center">
                        <div 
                          className="w-10 h-10 rounded-full flex items-center justify-center text-white text-lg font-bold mb-2"
                          style={{ background: config?.color }}
                        >
                          {config?.icon}
                        </div>
                        <span className="font-semibold text-sm mb-1">{config?.label}</span>
                        <p className="text-xs text-gray-600 line-clamp-2">{entry.plain_explanation}</p>
                      </div>
                    ) : (
                      <div className="text-center text-gray-400">
                        <span className="text-2xl">—</span>
                      </div>
                    )}
                  </div>
                );
              })}
            </div>
          ))}
        </div>
      )}

      {/* Empty state */}
      {!loading && selected.length < 2 && (
        <div className="text-center py-16 card">
          <span className="text-6xl mb-4 block">🌍</span>
          <h2 className="text-xl font-semibold mb-2">Selecione pelo menos 2 países</h2>
          <p className="text-gray-500">A comparação será exibida automaticamente</p>
        </div>
      )}

      {/* Legend */}
      {comparisons.length > 0 && (
        <div className="mt-8 flex flex-wrap justify-center gap-6">
          {Object.entries(statusConfig).map(([key, config]) => (
            <div key={key} className="flex items-center gap-2">
              <div 
                className="w-6 h-6 rounded-full flex items-center justify-center text-white text-xs"
                style={{ background: config.color }}
              >
                {config.icon}
              </div>
              <span className="text-sm">{config.label}</span>
            </div>
          ))}
        </div>
      )}
    </main>
  );
}
